...
pub async fn compose_http_response(
    ...
    if server_function::Debug == http_server_function {
        ...
        http_response_struct.addDropHeaders();
        ...
        if HttpRequestType::GET != parse_return_data.HttpRequestType {
            ...
            if contains_prohibited {
                ...
                match File::open(path).await {
                    Ok(o) => {
                        let mut buf_reader: BufReader<File> = BufReader::new(o);

                        let mut buffer: String = String::new();

                        let read_status = buf_reader.read_to_string(&mut buffer).await;
                    ...
                    Err(e) => compose_server_error().await,
                }
            }
        }
    } else {
...
pub async fn compose_server_error() -> Vec<u8> {
    let mut http_response_struct = HttpResponseStruct::new();
