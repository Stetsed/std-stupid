...
pub async fn parse_http_connection(
    input_connection_data: &[u8],
    read_counter: Option<usize>
) -> Result<ParseReturnData, StdStupidError> {
    // Find the / to find the method being used
    let mut connection_data = input_connection_data.to_vec();
    let length = read_counter.unwrap_or(connection_data.len());
    if length == 0 {
        return Err(HttpServerError::new("Length  is > 0").into());
    }
    connection_data.truncate(length);
    ...
    while connection_data.len() > 2 {
        ...
        let header_double_period_location =
            findSubStringWithBytes(header_supplied.as_slice(), b":")?;
